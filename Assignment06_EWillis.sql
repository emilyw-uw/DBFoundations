...
-- Change Log: 
-- 2022-08-15, EWillis, Created File
-- 2022-08-15, EWillis, Completed q's 1-3
-- 2022-08-16, EWillis, Completed q's 4-9
-- 2022-08-17, EWillis, Completed File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment06DB_EWillis')
	 Begin 
	  Alter Database [Assignment06DB_EWillis] set Single_user With Rollback Immediate;
	  Drop Database Assignment06DB_EWillis;
	 End
	Create Database Assignment06DB_EWillis;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment06DB_EWillis;
...
-- Create views using the select statements:
...
----Combine the results using table aliases:
...
--go
--Select Concat(EmployeeFirstName, ' ', EmployeeLastName) As EmployeeName From vEmployees;
--go
...
----Combine the results and use table aliases:
--Select CategoryName, ProductName, InventoryDate, Count From vCategories As c
--Join vProducts As p On c.CategoryID = p.CategoryID
--Join vInventories As i On p.ProductID = i.ProductID;
--go

----Order the results:
--Select Top 10000000 CategoryName, ProductName, InventoryDate, Count From vCategories As c
--Join vProducts As p On c.CategoryID = p.CategoryID
--Join vInventories As i On p.ProductID = i.ProductID
...
----Write subquery to get ProductID for 'Chai' and 'Chang':
--Select ProductID From vProducts 
--Where ProductName Like 'Chai' 
--	Or ProductName Like 'Chang';
--go 
...
----Look at just the data we want:
--Select 
--	CONCAT(EmployeeFirstName, ' ', EmployeeLastName) as Manager, 
--	CONCAT(EmployeeFirstName, ' ', EmployeeLastName) as Employee 
--From Employees;
--go
...
--	CONCAT(Emp.EmployeeFirstName, ' ', Emp.EmployeeLastName) as Employee 
...
Select Emp.EmployeeID, 
	CONCAT(Emp.EmployeeFirstName, ' ', Emp.EmployeeLastName) as Employee, 
	CONCAT(Mgr.EmployeeFirstName, ' ', Mgr.EmployeeLastName) as Manager 
From vEmployees as Emp Join vEmployees as Mgr
On Mgr.EmployeeID = Emp.ManagerID
Order By 1,2;
go
...
Select c.CategoryID, 
	   ...
	   e.EmployeeID, 
	   Employee = CONCAT(e.EmployeeFirstName, ' ', e.EmployeeLastName),
	   Manager = CONCAT(Mgr.EmployeeFirstName, ' ', Mgr.EmployeeLastName)
From vCategories as c Join vProducts as p
On c.CategoryID = p.CategoryID
Join vInventories as i On p.ProductID = i.ProductID
Join vEmployees as e On i.EmployeeID = e.EmployeeID
Join vEmployees as Mgr On Mgr.EmployeeID = e.ManagerID;
go
